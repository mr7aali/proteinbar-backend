  ...
  await Promise.all(
    defaults.map(async (page) => {
      ...
      const currentSectionKeys = new Set(
        currentSections
          .map((section) => String((section as Record<string, unknown>).sectionKey ?? "").trim())
          .filter(Boolean)
      );
...
export const adminService = {
  ...
  async listMonthlyPlanSubscriptionsAdmin() {
    ...
    return adminSubscriptions.map((item) => {
      ...
      const customer =
        customerSubscription.customer && typeof customerSubscription.customer === "object"
          ? (customerSubscription.customer as Record<string, unknown>)
      ...
      const selectedMeals = Array.isArray(selection.selectedMeals)
        ? (selection.selectedMeals as Array<Record<string, unknown>>)
        : [];
